...
UniswapPriceOracle.prototype.getTokenPrice = async function (
  uniPairAddress,
  denominationToken,
  blockNumber
) {
  const getAccumulatorValue = async (innerBlockNumber, timestamp) => {
    const token0 = await this.ethGetStorageAt(
      uniPairAddress,
      6n,
      innerBlockNumber
    );
    const token1 = await this.ethGetStorageAt(
      uniPairAddress,
      7n,
      innerBlockNumber
    );
    const reservesAndTimestamp = await this.ethGetStorageAt(
      uniPairAddress,
      8n,
      innerBlockNumber
    );
    const accumulator0 = await this.ethGetStorageAt(
      uniPairAddress,
      9n,
      innerBlockNumber
    );
    const accumulator1 = await this.ethGetStorageAt(
      uniPairAddress,
      10n,
      innerBlockNumber
    );
    ...
    if (token0 !== denominationToken && token1 !== denominationToken)
      throw new Error(
        `Denomination token ${addressToString(
          denominationToken
        )} is not one of the tokens for exchange`
      );
    if (reserve0 === 0n)
      throw new Error(
        `Exchange ${addressToString(
          uniPairAddress
        )} does not have any reserves for token0.`
      );
    if (reserve1 === 0n)
      throw new Error(
        `Exchange ${addressToString(
          uniPairAddress
        )} does not have any reserves for token1.`
      );
    ...
    const denominatorReserve =
      token0 === denominationToken ? reserve1 : reserve0;
    const accumulator =
      token0 === denominationToken ? accumulator1 : accumulator0;
    const timeElapsedSinceLastAccumulatorUpdate =
...
const getBlockByNumberFactory = (provider) => {
  ...
  return async (blockNumber) => {
    const stringifiedBlockNumber =
      typeof blockNumber === "bigint"
        ? `0x${blockNumber.toString(16)}`
        : blockNumber;
    const block = await requestProvider.request("eth_getBlockByNumber", [
...
const getStorageAtFactory = (provider) => {
  ...
  return async (address, position, block) => {
    ...
    const encodedBlockTag =
      block === "latest" ? "latest" : bigintToHexQuantity(block);
    const result = await requestProvider.request("eth_getStorageAt", [
      encodedAddress,
      encodedPosition,
      encodedBlockTag,
    ]);
...
const normalizeProvider = (provider) => {
  if ("request" in provider) {
          ...
          provider.sendAsync(
            ...
            (error, response) => {
              if (error !== null && error !== undefined)
                return reject(
                  unknownErrorToJsonRpcError(error, {
                    request: { method, params },
                  })
                );
              if (!isJsonRpcLike(response))
                return reject(
                  new Error(
                    -32000,
                    `Received something other than a JSON-RPC response from provider.sendAsync.`,
    ...
    throw new Error(
      `expected an object with a 'request', 'sendAsync' or 'send' method on it but received ${JSON.stringify(
        provider
      )}`
    );
  }
...
const assertProperty = (maybe, propertyName, expectedPropertyType) => {
  ...
  if (
    ...
    return;
  throw new Error(
    `Object.${propertyName} is of type ${typeof propertyValue} instead of expected type ${expectedPropertyType}`
  );
};
...
export default UniswapPriceOracle;
